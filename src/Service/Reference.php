...
class Reference extends Brightree
{
	...
	{
		parent::__construct();
		$this->wsdl = $this->config->service['reference'] .'?singleWsdl';
		$this->options = array('login' => $this->config->user['name'],'password' => $this->config->user['pass'],'uri' => $this->config->service['reference'],'location' => $this->config->service['reference'],'trace' => 1);
	}
